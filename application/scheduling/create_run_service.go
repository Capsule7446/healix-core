...
func validateStoredCreateRunResult(stored StoredCreateRunResult, command CreateRunCommand) error {
	...
	if stored.Snapshot.SchemaVersion() != execution.RunSnapshotSchemaCurrent || stored.Snapshot.RunID() != command.RunID || stored.Snapshot.TestTaskID() != command.TestTaskID || stored.Snapshot.TestTaskVersionID() != command.TestTaskVersionID || stored.Snapshot.Environment().ID != command.EnvironmentID {
