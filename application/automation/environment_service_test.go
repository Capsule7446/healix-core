...
func TestEnvironmentServiceLifecycleUsesRevisionCAS(t *testing.T) {
	...
	created, err := service.Create(context.Background(), domain.Environment{ID: "environment", DisplayName: "Local", BaseURL: "https://example.com", Variables: domain.EnvironmentVariables{}, CreatedAt: 1, UpdatedAt: 1})
	...
	updated, err := service.Update(context.Background(), created.ID, "CI", "https://ci.example.com", domain.EnvironmentVariables{"region": parameter.TextValue("eu")}, 1, 2)
...
func TestEnvironmentServiceRejectsConflictsAndPropagatesErrors(t *testing.T) {
	...
	valid := domain.Environment{ID: "environment", DisplayName: "Local", BaseURL: "https://example.com", Variables: domain.EnvironmentVariables{}, CreatedAt: 1, UpdatedAt: 1, Revision: 1}
	tests := []struct {
		name       string
		repository *environmentRepositoryFake
		create     bool
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.create {
				...
				_, err = service.Update(context.Background(), valid.ID, "Updated", valid.BaseURL, domain.EnvironmentVariables{}, expected, 2)
			}
