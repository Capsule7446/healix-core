...
func TestCompilePlanIndexesInvocationsOnceAcrossEntries(t *testing.T) {
	allocationsForEntries := func(entryCount int) float64 {
		...
		draft.Entries = make([]execution.Entry, entryCount)
		for index := range draft.Entries {
			draft.Entries[index] = execution.Entry{
				ID:                mustEntryID(fmt.Sprintf("execution-%03d", index)),
...
func TestCompilePlanRejectsUnsealedZeroValue(t *testing.T) {
	_, err := CompilePlan(execution.InstanceSnapshot{})
	if err == nil {
...
func TestCompilerRequiresConcreteRootAndNestedInvocations(t *testing.T) {
	...
	if _, err := compileSnapshotDraft(plan.Snapshot(), execution.InstanceSnapshot{}); err == nil {
...
func TestCompilePlanBuildsLockedWorkflowTreeAndBindsChildDefaults(t *testing.T) {
	...
	plan := execution.PlanSnapshot{
		InstanceID: mustInstanceID("exec"), Entries: []execution.Entry{{ID: mustEntryID("execution-entry"), TestTaskItemID: "task-item", FlowFragmentID: "root", WorkflowVersionID: "root-v1"}},
		Workflows: []execution.WorkflowSnapshot{
			...
			{FlowFragmentID: "child", VersionID: "child-v1", DisplayName: "子流程", VersionNumber: 1,
				...
				Steps:      []execution.Step{{ID: "click", DisplayName: "点击", Kind: execution.ActionStep, Action: "click", ElementTargetID: compilerNodeID, ElementTargetVersionID: compilerNodeV1}}},
		},
...
func TestCompilePlanCreatesDistinctRuntimeIdentitiesForSharedChildInvocations(t *testing.T) {
	...
	plan := execution.PlanSnapshot{InstanceID: mustInstanceID("exec"), FailurePolicy: execution.FailurePolicyStopOnFailure, Entries: []execution.Entry{{ID: mustEntryID("execution-entry"), TestTaskItemID: "task-item", SequenceNumber: 1, FlowFragmentID: "root", WorkflowVersionID: "root-v1"}}, Workflows: []execution.WorkflowSnapshot{
...
func TestCompilePlanKeepsTwoVersionsOfSameNodeExact(t *testing.T) {
	plan := execution.PlanSnapshot{InstanceID: mustInstanceID("exec"), FailurePolicy: execution.FailurePolicyStopOnFailure, Entries: []execution.Entry{{ID: mustEntryID("execution-entry"), TestTaskItemID: "task-item", SequenceNumber: 1, FlowFragmentID: "checkout", WorkflowVersionID: "checkout-v1"}},
		Workflows: []execution.WorkflowSnapshot{{FlowFragmentID: "checkout", VersionID: "checkout-v1", DisplayName: "结账", VersionNumber: 1, Steps: []execution.Step{
			{ID: "old", DisplayName: "旧版", Kind: execution.ActionStep, Action: "click", ElementTargetID: compilerNodeID, ElementTargetVersionID: compilerNodeV1},
			{ID: "new", DisplayName: "新版", Kind: execution.ActionStep, Action: "click", ElementTargetID: compilerNodeID, ElementTargetVersionID: compilerNodeV2},
...
func TestCompilePlanBuildsValidationGroup(t *testing.T) {
	member := execution.Step{ID: "member", DisplayName: "状态成功", Kind: execution.ValidationStep, ElementTargetID: compilerNodeID, ElementTargetVersionID: compilerNodeV1, Validation: &execution.Validation{Kind: "text_equals", Expected: "成功"}}
	plan := execution.PlanSnapshot{
		InstanceID: mustInstanceID("exec"), Entries: []execution.Entry{{ID: mustEntryID("execution-entry"), TestTaskItemID: "task-item", FlowFragmentID: "validation", WorkflowVersionID: "validation-v1"}},
		Workflows: []execution.WorkflowSnapshot{{
...
func TestCompilePlanRejectsMissingSnapshotAndCycles(t *testing.T) {
	missing := execution.PlanSnapshot{InstanceID: mustInstanceID("exec"), FailurePolicy: execution.FailurePolicyStopOnFailure, Entries: []execution.Entry{{ID: mustEntryID("execution-entry"), TestTaskItemID: "task-item", SequenceNumber: 1, FlowFragmentID: "root", WorkflowVersionID: "root-v1"}}, Workflows: []execution.WorkflowSnapshot{{FlowFragmentID: "root", VersionID: "root-v1", DisplayName: "root", VersionNumber: 1, Steps: []execution.Step{{ID: "click", DisplayName: "click", Kind: execution.ActionStep, Action: "click", ElementTargetID: compilerNodeID, ElementTargetVersionID: compilerNodeV1}}}}}
	if _, err := compileDraft(missing); err == nil {
		t.Fatal("missing node snapshot was accepted")
	}
	cycle := execution.PlanSnapshot{InstanceID: mustInstanceID("exec"), FailurePolicy: execution.FailurePolicyStopOnFailure, Entries: []execution.Entry{{ID: mustEntryID("execution-entry"), TestTaskItemID: "task-item", SequenceNumber: 1, FlowFragmentID: "a", WorkflowVersionID: "a-v1"}}, Workflows: []execution.WorkflowSnapshot{
...
func compilerNodeSnapshot(versionID, selector string) execution.NodeSnapshot {
	return execution.NodeSnapshot{ElementTargetID: compilerNodeID, VersionID: versionID, DisplayName: "节点", Selectors: []fingerprint.Selector{{Type: fingerprint.SelectorTestID, Value: selector}}, Fingerprint: fingerprint.Fingerprint{Tag: "button", Attributes: map[string]string{}}}
}

func compileDraft(draft execution.PlanSnapshot) (CompiledEntry, error) {
...
func TestCompilePlanKeepsRepeatedEntryOccurrencesIndependent(t *testing.T) {
	draft := minimalCompilerPlan()
	draft.Entries = []execution.Entry{
		{ID: mustEntryID("execution-a"), TestTaskItemID: "item-a", SequenceNumber: 1, FlowFragmentID: "root", WorkflowVersionID: "root-v1"},
		{ID: mustEntryID("execution-b"), TestTaskItemID: "item-b", SequenceNumber: 2, FlowFragmentID: "root", WorkflowVersionID: "root-v1"},
	}
	plan, err := execution.Seal(draft)
	if err != nil {
		t.Fatal(err)
