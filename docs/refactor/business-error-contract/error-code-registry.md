...
| `EXECUTION_RUNTIME_CONFIGURATION_INVALID` | `FAILED_PRECONDITION` | `execution runtime configuration is invalid` | none | Repair the engine coordinator's own configuration (validated dependencies and settings) before starting; no dependency identity or setting value is exposed. |
| `EXECUTION_SCHEDULING_ADAPTER_UNAVAILABLE` | `UNAVAILABLE` | `scheduling adapter is unavailable` | none | Retry once the recorder, claim, release, state, decision, or queue port is reachable again; adapter and identity details remain private. |
| `EXECUTION_ENTRY_EXECUTOR_CONFIGURATION_INVALID` | `FAILED_PRECONDITION` | `entry executor configuration is invalid` | none | Supply the entry executor's required constructor dependencies before use; no dependency identity is exposed. |
| `EXECUTION_ENTRY_BROWSER_SESSION_ADAPTER_CONTRACT_VIOLATION` | `INTERNAL` | `browser session adapter returned an invalid outcome` | none | The host browser session factory returned a nil or otherwise invalid session; preserve the cause only privately and expose no session or execution identity. |
| `EXECUTION_STEP_TRANSITION_COMMIT_PAYLOAD_TOO_LARGE` | `OUT_OF_RANGE` | `step transition commit payload is too large` | none | Reduce the commit payload below its byte budget before retrying; the measured size and payload contents remain private. |
| `EXECUTION_STEP_TRANSITION_COMMIT_RUN_MISMATCH` | `FAILED_PRECONDITION` | `step transition commit does not match the claimed run` | none | Re-read the authoritative claimed run before retrying; run identifiers remain private. |
...
| `SAMPLING_PUBLICATION_AUTHORITY_INVALID` | `INVALID_ARGUMENT` | `sampling publication authority is invalid` | none | Boundary classification for `MapSamplingPublication`/`mapSamplingNode`: correct the resolution mode, element target references, or authority fields before retrying; temporary and formal element target identities and resolution modes remain private on the classified cause. Produced from `application/automation`; the `SAMPLING_*` prefix is authoritative per `v0.5-error-inventory.md:37`. |
| `SAMPLING_PUBLICATION_COMMAND_INVALID` | `INVALID_ARGUMENT` | `sampling publication command is invalid` | none | Correct the sampling publication transaction command before retrying; command and payload details remain private. Produced from `application/automation`; the `SAMPLING_*` prefix is authoritative per `v0.5-error-inventory.md:37`. |
