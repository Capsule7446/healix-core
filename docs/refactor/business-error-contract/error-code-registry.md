...
| `EXECUTION_CREATE_INSTANCE_PLAN_INVALID` | `INVALID_ARGUMENT` | `create-instance plan is invalid` | none | Boundary-classified at `domain/execution`'s `Draft.Validate`/`Seal`: repair the referenced execution plan and retry. Internal shape, budget, and dependency detail plus every identity remain private on the cause; an already-classified step-shape, fingerprint, or parameter fault reached mid-walk passes through unchanged instead of being buried under this code. |
| `EXECUTION_CREATE_INSTANCE_STEP_SHAPE_INVALID` | `INVALID_ARGUMENT` | `create-instance step shape is invalid` | ordered typed violations only | Aggregate validation envelope for one workflow snapshot's step tree. Recursive step, branch, and validation-group failures share the unindexed `steps` field per the existing recursive-walk precedent; parameter and top-level fields use their own logical path. An already-classified interpolation or parameter-binding fault reached mid-walk propagates unchanged rather than degrading into a violation. |
| `EXECUTION_CREATE_INSTANCE_SNAPSHOT_INVALID` | `INVALID_ARGUMENT` | `create-instance snapshot is invalid` | none | Boundary-classified at `SealRunSnapshot`, `HydrateRunSnapshot`, and `NewRun`: repair the referenced run snapshot input and retry. Identities and raw causes remain private; an already-classified plan, step-shape, or environment-snapshot fault reached while validating the snapshot passes through unchanged. |
| `EXECUTION_ENVIRONMENT_SNAPSHOT_INVALID` | `INVALID_ARGUMENT` | `environment snapshot is invalid` | ordered typed violations only | Aggregate validation envelope for the run's environment, screenshot, and healer policy snapshots. Environment property and variable names are caller-declared and never reach public text; every failing check degrades into a generic violation under the `environment.*` field family. |
...
| `AUTOMATION_EXECUTION_FLOW_HISTORY_INVALID` | `FAILED_PRECONDITION` | `execution flow version history is invalid` | ordered typed violations only | The history itself — ordering, uniqueness, the source chain, and agreement between the current version and the latest one — rather than the shape of any single version, which keeps its own code and propagates unwrapped. `FAILED_PRECONDITION` because the remediation is to repair persisted history, not to correct a field the caller just supplied. Version indexes are 0-based; no version identity reaches public text. |
| `AUTOMATION_EXECUTION_FLOW_DEPENDENCY_INVALID` | `INVALID_ARGUMENT` | `execution flow dependency resolution is invalid` | ordered typed violations only | Dependency resolution: the workflow and element target snapshot sets, the reference resolution set, the reference graph including cycles and unresolved versions, and the bindings between them. Separate from the field-level execution flow code because a caller repairs a broken dependency graph by re-resolving the catalog, not by editing a field. A contained `PARAMETER_*` failure propagates unwrapped rather than being restated. Graph paths carry no index: the reference walk is recursive and has no flat position to report. |
| `AUTOMATION_ELEMENT_TARGET_INVALID` | `INVALID_ARGUMENT` | `element target content is invalid` | ordered typed violations only | Aggregate validation envelope for an element target's identity, properties, current version pointer, selectors, fingerprint, and version source. Never one code per failing field, never a joined message. Replaces the previous joined-string `ElementTargetAggregate.Validate`. |
| `AUTOMATION_ELEMENT_TARGET_HISTORY_INVALID` | `FAILED_PRECONDITION` | `element target version history is invalid` | ordered typed violations only | The version history itself — ordering, uniqueness, ownership, and the publication identity checks that gate a new version — rather than the shape of any one version, which keeps its own code and propagates unwrapped. `FAILED_PRECONDITION` because the remediation is to repair persisted history or supply a fresh version identity, not to correct a field the caller just supplied. Short-circuit single-violation envelopes are acceptable; full aggregation is not required. No version identity reaches public text. |
| `AUTOMATION_FLOW_FRAGMENT_INVALID` | `INVALID_ARGUMENT` | `flow fragment content is invalid` | ordered typed violations only | Aggregate validation envelope for a flow fragment's identity, properties, current version pointer, and step tree, including validation steps and validation groups. Parameter definitions are flow fragment content and degrade into this same envelope; a parameter default's own `PARAMETER_*` fault keeps its code. Never one code per failing field, never a joined message. Replaces the previous joined-string `FlowFragmentAggregate.Validate`. |
| `AUTOMATION_FLOW_FRAGMENT_HISTORY_INVALID` | `FAILED_PRECONDITION` | `flow fragment version history is invalid` | ordered typed violations only | Mirrors `AUTOMATION_ELEMENT_TARGET_HISTORY_INVALID` for the flow fragment aggregate family. |
| `AUTOMATION_ENVIRONMENT_INVALID` | `INVALID_ARGUMENT` | `environment content is invalid` | ordered typed violations only | Aggregate validation envelope for an environment's identity, base URL, and typed variables. Variable names and values never reach public text. Replaces the previous joined-string `Environment.Validate`. |
| `AUTOMATION_AGGREGATE_TRANSITION_INVALID` | `FAILED_PRECONDITION` | `automation aggregate transition is invalid` | ordered typed violations only | Single shared helper for the empty-transition and bad-timestamp checks common to every automation aggregate — environment, element target, flow fragment, and execution flow — mirroring how `AUTOMATION_AGGREGATE_DELETED` is one cross-aggregate helper rather than four copies. `FAILED_PRECONDITION` because the remediation is to reach a valid prior state (a fresh timestamp, a non-terminal lifecycle state, a new version identity) before retrying. Single-violation envelopes; no timestamp or identity value reaches public text. |
...
| `AUTOMATION_SAMPLING_PUBLICATION_CONTENT_INVALID` | `INVALID_ARGUMENT` | `sampling publication content is invalid` | none | The publication's own content shape: node resolution modes, referenceable versions, merge and reuse authority, and step references without a matching decision. Distinct from the four transaction-level sampling publication codes below, which cover digest, availability, adapter contract, and authority. Classified once at the single exported boundary; the individual checks stay ordinary Go errors on the private cause, and no temporary or formal identity reaches public text. |
