...
| `EXECUTION_STEP_CONFIGURATION_INVALID` | `INVALID_ARGUMENT` | `step configuration is invalid` | ordered typed violations only | Every step-definition shape failure — action, wait/repeat, assertion kind, workflow-call target/binding, missing driver dependency — distinguished by violation field path, never by a separate code per shape. URL schemes, action kinds, assertion kinds, and parameter names are caller-declared and never echoed publicly. |
| `EXECUTION_STEP_PHASE_TRANSITION_INVALID` | `FAILED_PRECONDITION` | `step phase transition is invalid` | none | Covers the StepExecution phase machine (`CanTransition`/`ValidatePhaseTransition`) and the leaf lifecycle start boundary. Node identity and phase names remain private. |
| `EXECUTION_HEALING_REFUSED` | `FAILED_PRECONDITION` | `healing was refused` | none | A healing policy's own refusal decision — safety rejection or no candidate reaching the review cap. Never an adapter failure encountered while healing; the safety explanation remains private. |
| `EXECUTION_EVIDENCE_RECORD_FAILED` | `UNAVAILABLE` | `execution evidence could not be recorded` | none | A Facts-port recording failure: `rt.emit`, heal-sample staging, and heal-decision staging. Distinct from `EXECUTION_TRANSIENT_DRIVER`, which classifies the Driver port, not the Facts port. |
