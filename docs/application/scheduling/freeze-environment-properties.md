...
在创建执行实例时冻结所选 Environment 的类型化 `Variables`，并在执行时通过 `env.` 作用域提供；Core 不区分、授权或解析 secret。新执行实例默认写 Current（当前为 `InstanceSnapshotSchemaV2`）；底层调用方可显式选择 V1 或 V2，V1 字符串按 TEXT 读取且保持原摘要。
...
- 密封在 `execution.InstanceSnapshot` 中的 `EnvironmentSnapshot`。
- 执行入口的 `env.` 变量作用域。
...
sequenceDiagram
    ...
    participant Runtime
    Host->>Scheduling: CreateInstance(environment selection)
...
- [`application/scheduling/create_instance_builder.go`](../../../application/scheduling/create_instance_builder.go)：创建执行实例时组装输入。
