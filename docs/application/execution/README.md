...
- 执行引擎负责通过 `engine.CompilePlan` 把已冻结的 `execution.RunSnapshot` 编译为带私有 Program 与身份封印的 `CompiledEntry`，再由 `engine.RunProgram` 运行；宿主组合层将该能力接入 `EntryRunner`。`EntryExecutor` 只拥有所提供顶层执行项的浏览器会话生命周期，不拥有快照编译、队列、领取执行权、执行实例状态或执行证据持久化。
