...
- 领域转换：`NodeAggregate.Restore`。恢复节点但不重写版本历史。
...
使用 `NodeRepository.Load/SaveAggregate`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。恢复节点但不重写版本历史。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as NodeService.Restore
