...
- 方法：`TestTaskService.SavePublished(context.Context, aggregate、expected)`
- 输出：`domain.TestTaskAggregate`；只有端口提交成功才返回成功结果。
- 领域转换：`aggregate.Validate`。调用方构造发布结果；Revision 必须恰为 expected.Next()。
...
使用 `TestTaskRepository.Load/SaveAggregate`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。调用方构造发布结果；Revision 必须恰为 expected.Next()。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as TestTaskService.SavePublished
