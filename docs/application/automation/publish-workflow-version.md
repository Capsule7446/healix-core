...
- 方法：`WorkflowService.PublishVersion(context.Context, id、versionID、definition、expected、at)`
- 输出：`domain.WorkflowAggregate`；只有端口提交成功才返回成功结果。
- 领域转换：`WorkflowAggregate.PublishVersion`。校验步骤定义，追加版本并切换 CurrentVersionID。
...
使用 `WorkflowRepository.Load/SaveAggregate`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。校验步骤定义，追加版本并切换 CurrentVersionID。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as WorkflowService.PublishVersion
    ...
    alt 读取失败或 Revision 冲突
        ...
        S->>D: WorkflowAggregate.PublishVersion
        alt 领域校验失败
            D-->>S: domain error
            S-->>A: error（不写入）
        else 转换成功
...
- [测试](../../../application/automation/asset_service_test.go)
