...
- 方法：`FolderService.Create(context.Context, folder、expected)`
- 输出：`FolderSnapshot`；只有端口提交成功才返回成功结果。
- 领域转换：`domain.NewFolderForest`。追加到森林副本后校验父子关系、种类和环。
...
使用 `FolderRepository.Load/Save`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。追加到森林副本后校验父子关系、种类和环。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as FolderService.Create
