...
使用 `Candidate/Verifier/Authorizer/Clock/CommitRejection`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。不加载节点；以候选 Revision CAS 写入拒绝及可信审核元数据。
...
- [端口](../../../application/automation/heal_candidate_repository.go)
