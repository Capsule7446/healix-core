...
- 领域转换：`domain.NewEnvironment`。构造器初始化 Revision=1；创建不预读且无 CAS。
...
使用 `EnvironmentRepository.Create`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。构造器初始化 Revision=1；创建不预读且无 CAS。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as EnvironmentService.Create
