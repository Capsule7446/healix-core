...
- 方法：`WorkflowService.Delete(context.Context, id、expected、at)`
- 输出：`domain.WorkflowAggregate`；只有端口提交成功才返回成功结果。
- 领域转换：`WorkflowAggregate.Delete`。软删除聚合，保留定义版本。
...
使用 `WorkflowRepository.Load/SaveAggregate`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。软删除聚合，保留定义版本。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as WorkflowService.Delete
