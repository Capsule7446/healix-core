...
- 方法：`WorkflowService.Create(context.Context, workflow、initial WorkflowVersion)`
- 输出：`domain.WorkflowAggregate`；只有端口提交成功才返回成功结果。
- 领域转换：`domain.NewWorkflow`。校验首版定义及步骤并建立当前版本；无 CAS。
...
使用 `WorkflowRepository.Create`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。校验首版定义及步骤并建立当前版本；无 CAS。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as WorkflowService.Create
