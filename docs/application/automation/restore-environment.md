...
- 领域转换：`Environment.Restore`。清除 DeletedAt；未删除对象不可恢复。
...
使用 `EnvironmentRepository.Load/Update`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。清除 DeletedAt；未删除对象不可恢复。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as EnvironmentService.Restore
