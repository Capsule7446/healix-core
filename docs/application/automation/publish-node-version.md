...
- 方法：`NodeService.PublishVersion(context.Context, id、versionID、pageURL、origin、selectors、fingerprint、source、expected、at)`
- 输出：`domain.NodeAggregate`；只有端口提交成功才返回成功结果。
- 领域转换：`NodeAggregate.PublishVersion`。追加不可变版本，校验选择器/指纹/来源并切换当前版本。
...
使用 `NodeRepository.Load/SaveAggregate`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。追加不可变版本，校验选择器/指纹/来源并切换当前版本。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as NodeService.PublishVersion
    ...
    alt 读取失败或 Revision 冲突
        ...
        S->>D: NodeAggregate.PublishVersion
        alt 领域校验失败
            D-->>S: domain error
            S-->>A: error（不写入）
        else 转换成功
