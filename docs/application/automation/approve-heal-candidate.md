...
- 方法：`HealReviewService.Approve(context.Context, HealCandidateReviewCommand)`
- 输出：`domain.NodeAggregate`；只有端口提交成功才返回成功结果。
- 领域转换：`NodeAggregate.PublishVersion`。原子更新候选状态与节点版本；校验节点 Revision 和基线版本。
...
使用 `Candidate/Node/Verifier/Authorizer/Clock/CommitApproval`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。原子更新候选状态与节点版本；校验节点 Revision 和基线版本。
...
- [测试](../../../application/automation/heal_review_service_test.go)
