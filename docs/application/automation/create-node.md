...
- 方法：`NodeService.Create(context.Context, node、initial NodeVersion)`
- 输出：`domain.NodeAggregate`；只有端口提交成功才返回成功结果。
- 领域转换：`domain.NewNode`。同时校验节点与首版本并建立 CurrentVersionID；无 CAS。
...
使用 `NodeRepository.Create`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。同时校验节点与首版本并建立 CurrentVersionID；无 CAS。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as NodeService.Create
