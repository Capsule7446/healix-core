...
- 方法：`FolderService.Delete(context.Context, kind、id、expected)`
- 输出：`FolderSnapshot`；只有端口提交成功才返回成功结果。
- 领域转换：`FolderForest.RequireEmpty`。适配器须在同一事务中对森林与占用快照双重 CAS。
...
使用 `Load/Occupancy/DeleteEmptyFolder`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。适配器须在同一事务中对森林与占用快照双重 CAS。
...
sequenceDiagram
    actor A as 入站适配器
    participant S as FolderService.Delete
