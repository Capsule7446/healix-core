...
- 方法：`SamplingPublicationService.Publish(context.Context, publicationID、SamplingPublication)`
- 输出：`domain.SamplingPublicationResult`；只有端口提交成功才返回成功结果。
- 领域转换：`SamplingPublication.Validate`。publicationID 必填；幂等和原子发布由适配器负责。
...
使用 `SamplingPublicationRepository.Publish`。应用层的 Revision 比对只提供快速失败；仓储适配器必须在存储事务中以 `expected` 执行条件写，竞争窗口中的失败也应映射为冲突。publicationID 必填；幂等和原子发布由适配器负责。
...
sequenceDiagram
    ...
    alt 读取失败或 Revision 冲突
        ...
        S->>D: SamplingPublication.Validate
        alt 领域校验失败
            D-->>S: domain error
            S-->>A: error（不写入）
        else 转换成功
            ...
            alt 提交失败
                ...
                S-->>A: domain.SamplingPublicationResult
            end
        end
    end
```
