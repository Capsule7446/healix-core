...
一条本领域特有的历史：**机器可读的代码与位置不再走独立的 `ValidationIssues` 结构** —— 那个类型已从代码中删除。多字段校验一律产出**一个**顶层 `AUTOMATION_*` fault，位置信息由其中有序的 `fault.Violation` 承担。内部子校验（例如 `VersionSource.Validate`）保持普通 Go error，由拥有 code 的聚合边界降级为自己的一条 violation，因此越界枚举值不会外泄。
