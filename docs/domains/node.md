...
- **`Node`**（[`step.go`](../../domain/node/step.go)）只有 `ID` 与 `Run`。
- **`Program`**（[`runtime.go`](../../domain/node/runtime.go)）包含根节点和按规格 ID 索引的 `fingerprint.ElementTargetSpec`。
- **`Runtime`**（[`runtime.go`](../../domain/node/runtime.go)）持有 `InstanceID` / `EntryID` / `ClaimToken`、`Driver`、`Specs`、`SelectorOverlay`、`Scratchpad` 及一组可选端口。它**没有** `WorkerFence` 字段 —— 栅栏在每次写事实时由 `InstanceID` 和 `ClaimToken` 现场构造（[`runtime.go`](../../domain/node/runtime.go)）。
- **`StepExecution`** 管理阶段；`OperationRunner` / `Poller` 分离重试与轮询机制。
- **`ValidationGroupNode`** 表示 OR 分支，每个分支内部为 AND。
- **`TimelineMark`**（[`lifecycle.go`](../../domain/node/lifecycle.go)）以录制零点后的 `Offset + Sequence` 标记事件；`StepTimelineEvent` 只表达叶子步骤的 `STARTED` / `FINISHED`。
...
只有四类节点会开启叶子生命周期，因而只有它们产生步骤时间线和完成处理链 —— `beginLeafLifecycle` 全仓恰好四个调用点：`StepNode`（[`step.go`](../../domain/node/step.go)）、`WaitNode`（[`composite.go`](../../domain/node/composite.go)）、`ValidationNode`（[`validation.go`](../../domain/node/validation.go)）与 `ValidationGroupNode`（[`validation.go`](../../domain/node/validation.go)）。`WorkflowNode`、`WorkflowCallNode` 与 `RepeatNode` 只负责编排。

### 节点出现编号

编号按 `NodeID` 分配，支持嵌套同 ID 的 LIFO 栈。它会作为 `Occurrence` 随事件、观察与事实一路带进[执行证据](evidence.md)（`OperationObservation` 同时携带 `EntryID` 与 `Occurrence`，[`runtime.go`](../../domain/node/runtime.go)），`RepeatNode` 在同一 `NodeID` 上跑出的各轮因此事后可区分。取出编号走的是 nil 安全访问器（[`runtime.go`](../../domain/node/runtime.go)），最佳努力观察不会因取不到编号而打断执行。

两处容易记反的细节（均在 [`runtime.go`](../../domain/node/runtime.go)）：
...
阶段机由 [`stepPhaseTransitions`](../../domain/node/runtime.go)（`runtime.go`）穷举，共七个阶段，终态没有出边。注意 node 的 `Phase` 里**没有** `ABORTED` —— 那是 `domain/execution` 的顶层执行项状态，不是步骤阶段。
...
`ValidatePhaseTransition`（[`runtime.go`](../../domain/node/runtime.go)）向持久化适配器公开同一道保护，因此部分写入或重复写入无法造出不可能的 `StepExecution` 历史。
...
分类入口是 [`classifyNodeFault`](../../domain/node/fault_classification.go)（`fault_classification.go`），只有四条分支：
...
`EXECUTION_TRANSIENT_DRIVER` 不走这条分类，它由 `transientDriverFault`（[`fault_classification.go`](../../domain/node/fault_classification.go)）在 Driver 明确声明可重试时单独产出 —— 只有显式瞬态才可重试，是自愈不被误触发的前提。
...
四个显式超时常量：终态事件写 5 秒、operation observation 写 5 秒（[`runtime.go`](../../domain/node/runtime.go)），完成处理器单个 5 秒、整条完成链 30 秒（[`lifecycle.go`](../../domain/node/lifecycle.go)）。等待与验证尊重上下文。

导航 URL 在执行前校验，变量展开错误显式返回；敏感验证证据由目标/断言判断后避免记录值。重试次数、等待超时、轮询间隔、步骤间隔限制资源；执行程序规模由 `CompilePlan` 从冻结快照编译顶层执行项时约束。

## 交互

[`CompilePlan(snapshot execution.InstanceSnapshot)`](../../application/engine/compiler.go)（`compiler.go`）从不可变执行实例快照编译所有顶层执行项，当前引擎不以已封存 `Plan` 作为输入。`Program` 保持在带身份封印的 `CompiledEntry` 内部，只能交给 `RunProgram`。
