...
## 交互
Automation 用 Names 推导环境键；Node 用 Runtime/Scratchpad 或工作流绑定实现 Resolver，并在动作、参数和验证期展开。Execution 保存未解析模板，保证密封计划不含运行时秘密。Interpolation 不读取环境变量、Vault、数据库或浏览器。
...
- [Automation 环境键使用](../../domain/automation/test_task.go)、[Node 运行时使用](../../domain/node/step.go)、[工作流绑定](../../domain/node/composite.go)
