...
| `automation.Node`                   | 节点资产         | Automation    | 聚合根实体  | 可编辑并可发布不可变版本供引用的节点定义资产。 |
...
| `automation.TestTask`               | 测试任务         | Automation    | 聚合根实体  | 面向测试业务的版本化**执行定义**；组织待执行工作流，但不是一次实际执行。             |
| `automation.TestTaskVersion`        | 测试任务版本       | Automation    | 手工版本实体 | 测试任务某一版本的顶层执行项顺序、类型化参数、版本策略与失败策略；只由显式手工创作创建，Sampling/Heal 不会代为发布。 |
| `automation.TestTaskItem`           | 测试任务条目       | Automation    | 子实体    | 测试任务版本中的一个有序工作流顶层执行项。                                |
| `automation.TestTaskVersionPlan`    | 测试任务发布计划     | Automation    | 发布快照   | 发布时解析并锁定工作流、节点与工作流引用依赖的 Automation 产物。          |
| `sampling.SamplingWorkspace`        | 采样工作区        | Sampling      | 草稿工作区  | 保存采样草稿与重写状态；不同于承载浏览器交互生命周期的 `sampling.Session`。 |
| `sampling.Session`                  | 采样会话         | Sampling      | 会话实体   | 管理一次浏览器采样交互、capture 幂等与关闭状态；不是 SamplingWorkspace 的别名。 |
...
| `execution.WorkflowEntry`           | 顶层执行项       | Execution     | 计划条目   | 执行计划中按序运行的顶层工作流执行项。                                |
| `execution.WorkflowSnapshot`        | 工作流快照        | Execution     | 依赖快照   | 执行计划锁定的工作流定义。                                     |
| `execution.NodeSnapshot`            | 节点快照         | Execution     | 依赖快照   | 执行计划锁定的节点定义。                                         |
| `execution.Run`                     | 执行实例         | Execution     | 状态实体   | 一份执行的整体身份与状态；创建时解析所有 `latest` 并冻结执行实例快照。 |
| `execution.RunSnapshot`             | 执行实例快照         | Execution     | 已封存值快照 | 冻结测试任务、具体版本、环境 `Properties`、策略和完整调用作用域；运行时不再解析 `current`/`latest`。 |
...
| `execution.ExecutionStatus`         | 顶层执行项状态     | Execution     | 状态值    | 单个顶层执行项从待执行到终态的状态。                                       |
| `node.Node`                         | 可运行节点        | Node          | 行为接口   | 可由运行时执行的节点行为，不等同于 Automation 中的节点资产。                     |
| `node.Program`                      | 执行程序         | Execution     | 临时编译模型 | Engine 为一次顶层执行项执行构造的节点树与 `NodeSpec` 索引；不持久化、不跨执行复用。 |
...
| `TestTaskVersionPlan` 与 `execution.Plan` | 前者是 Automation 发布时锁定资产依赖的发布快照；后者是 Scheduling 加入 `RunID`、顶层执行项身份和运行输入后，在 Execution 中封存的单次执行计划。       |
| `Workflow` 与 `Program`                   | `Workflow` 是可编辑、版本化和发布的自动化资产；`Program` 是针对一次执行编译出的执行程序。                                            |
| Automation Node 与 Node domain            | `automation.Node` 是可持久化的版本化资产；`node.Node` 是运行时行为接口。两者处于不同上下文。                                    |
| `Run` 与顶层执行项                      | 执行实例表示整份计划的运行；顶层执行项表示执行实例中某个 `WorkflowEntry` 的执行及其状态。                                   |
...
| `latest` 与具体版本                           | `latest` 只是创作策略；创建执行实例时从一致目录视图解析并冻结 Node、Workflow、TestTask 的具体发布版本，运行时不重新解析。 |
...
1. Automation 保存“要执行什么”；TestTask 版本由显式手工创作创建。
2. Scheduling 在创建执行实例的一致目录/事务视图中解析所有 `latest`、Environment `Properties`、类型化值与绑定以及调用作用域。
3. Execution 封存不可变 `RunSnapshot`；运行期间不再查询 `current`/`latest`。
