...
| **仅端口契约** | Core 只定义接口、命令、结果或值对象；持久化、队列、浏览器和外部服务实现不在 Core 内。 |
| **适配器义务** | 宿主必须实现并保证事务、并发控制、幂等、授权、审计、驱动或投影等语义。 |
...
3. **区分 Core 与宿主。** 数据库、队列、浏览器、密钥后端、网络、文件系统、授权、审计和读模型等能力，应明确标为“仅端口契约”或“适配器义务”。
