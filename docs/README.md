...
- [测试台账索引](testing/TEST_CASES_INDEX.md)：各包 `TEST_CASES.md` 的入口与覆盖现状。台账里的源码链接与测试函数名由链接守卫强制，符号调整必须同步更新。
...
6. **Use Case 文档逐项维护。** 新增、调整名称或删除应用用例时，必须同步更新模块 README 与本页完整索引；不得只依赖目录发现。
...
11. **`docs/contracts/` 是代码的输入，不是代码的描述。** 错误码注册表、摘要 wire tag 清单和退役契约由 `architecture/` 守卫读取或交叉校验；错误码边界说明用于解释当前归属，不参与逐行解析。移动或调整受守卫读取的文件，必须同步对应测试中的路径常量。
