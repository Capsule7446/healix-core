...
`domain/weburl` 拥有 Core 中「绝对 HTTP(S) URL」这一条共享规则。该规则原本在五处各写一遍，并且已经漂移：两个 base URL 校验点缺控制字符检查，采样起始 URL 校验点缺 userinfo 与控制字符检查，navigation 校验点在存在插值时跳过 host 检查。规则收敛到这里之后，本表就是该规则的唯一判定处——属于某一个调用方的用例应留在该调用方的矩阵里，不进本表。
...
4. 新增 URL 规则只能加在 `Check` 里。任何调用方重新手写 scheme、host 或 userinfo 判断，都是这个包被建立起来所要终止的那次漂移。
