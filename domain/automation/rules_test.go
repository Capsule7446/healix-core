...
func TestVersionNumberAndHealStreakBounds(t *testing.T) {
	if _, err := NextVersionNumber([]VersionMeta{{ID: "max", VersionNumber: math.MaxInt}}); err != ErrVersionNumberOverflow {
		t.Fatalf("version overflow error = %v", err)
	}
	decision, err := (HealStreak{CandidateHash: "candidate", Count: math.MaxInt, Observing: true}).Observe(HealSucceeded, "candidate", true)
	if err != nil || decision.NextCount != 3 || !decision.Promote {
		t.Fatalf("saturated streak decision = %+v, %v", decision, err)
	}
	if _, err := (HealStreak{Count: -1, Observing: true}).Observe(HealSucceeded, "candidate", true); err == nil {
...
func TestHealCandidateReviewCommandValidation(t *testing.T) {
	command := HealCandidateReviewCommand{NodeID: "node-1", BaseNodeVersionID: "nv-1",
		CandidateHash: "candidate", PromotedVersionID: "nv-2", ExpectedNodeRevision: 1}
