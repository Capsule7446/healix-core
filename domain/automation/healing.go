...
func (command HealCandidateReviewCommand) Validate(approval HealApprovalStatus) error {
	...
	case HealApprovalApproved:
		if err := command.ExpectedNodeRevision.ValidatePersisted(); err != nil {
			return fmt.Errorf("heal candidate approval requires expected node revision: %w", err)
		}
...
func (streak HealStreak) Observe(outcome HealOutcome, candidateHash string, baseIsCurrent bool) (HealStreakDecision, error) {
	...
	if streak.Count < 0 {
		return HealStreakDecision{}, fmt.Errorf("heal streak count cannot be negative")
	}
	if !baseIsCurrent {
