...
func TestWorkflowAggregateValidateRejectsOversizedStepTrees(t *testing.T) {
	t.Run("depth", func(t *testing.T) {
		step := WorkflowStep{ID: "leaf", DisplayName: "叶子", Kind: StepAction, Action: "noop"}
	...
	t.Run("count", func(t *testing.T) {
		steps := make([]WorkflowStep, maxWorkflowStepCount+1)
		for index := range steps {
			steps[index] = WorkflowStep{ID: fmt.Sprintf("step-%d", index), DisplayName: "步骤", Kind: StepAction, Action: "noop"}
		}
