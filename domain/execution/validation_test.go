...
func TestSealedPlanAccessorsAndNestedCopies(t *testing.T) {
	...
	if plan.RunID() != mustInstanceID("run") || plan.Entries()[0].WorkflowVersionID != workflow.VersionID || len(plan.Workflows()) != 1 || len(plan.Nodes()) != 1 || len(plan.References()) != 0 {
...
func TestNavigateSealValidation(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.value, func(t *testing.T) {
			...
			draft := PlanSnapshot{RunID: mustInstanceID("run"), FailurePolicy: FailurePolicyStopOnFailure, Entries: []Entry{{ID: mustEntryID("execution-entry"), TestTaskItemID: "task-item", SequenceNumber: 1, FlowFragmentID: workflow.FlowFragmentID, WorkflowVersionID: workflow.VersionID}}, Workflows: []WorkflowSnapshot{workflow}}
			err := draft.Validate()
...
func validDraftWithNodes(nodes ...NodeSnapshot) PlanSnapshot {
	...
	return PlanSnapshot{RunID: mustInstanceID("run"), FailurePolicy: FailurePolicyStopOnFailure, Entries: []Entry{{ID: mustEntryID("execution-entry"), TestTaskItemID: "task-item", SequenceNumber: 1, FlowFragmentID: workflow.FlowFragmentID, WorkflowVersionID: workflow.VersionID}}, Workflows: []WorkflowSnapshot{workflow}, Nodes: nodes}
}
...
func workflowReferenceDraft(edges map[string]string) PlanSnapshot {
	...
	return PlanSnapshot{RunID: mustInstanceID("run"), FailurePolicy: FailurePolicyStopOnFailure, Entries: []Entry{{ID: mustEntryID("execution-entry"), TestTaskItemID: "task-item", SequenceNumber: 1, FlowFragmentID: "a", WorkflowVersionID: "a-v1"}}, Workflows: workflows, References: resolutions}
}
