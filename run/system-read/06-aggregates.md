...
- **WorkflowAggregate** (`domain/automation/assets.go:299`): workflow identity, references, steps and versions. Invariants include version/reference consistency and validated workflow structure.
- **TestTaskAggregate** (`domain/automation/test_task_types.go:34`): task identity, items, versions and execution plans. It governs what is runnable and dependency snapshots.
...
- **Evidence records** (`domain/automation/evidence.go`): immutable facts for replay/review, but persistence ownership is external.
